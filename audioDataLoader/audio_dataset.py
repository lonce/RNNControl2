...
class MuLawAudioDataset2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_seq = mu_law_encode(chunk[1:], quantization_channels=256).long()
        ...
        if self.config.encode:
            encoded_input = mu_law_encode(input_chunk, quantization_channels=256)
            input_seq = encoded_input.float() / 255.0
        else:
